Public Class ucEditAlumni

    Public AlumniObj As AlumniController
    Public AddressObj As AlumniAddressController
    Public ContactObj As ContactPointsController
    Public EmploymentObj As EmploymentController
    ...
    Private Sub ucEditAlumni_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        'Load information for edit

        'Load Alumni Info
        txtFirstName.Text = AlumniObj.FirstName
        txtMiddleName.Text = AlumniObj.MiddleName
        txtLastName.Text = AlumniObj.LastName
        txtExtName.Text = AlumniObj.ExtName
        cboGender.SelectedIndex = cboGender.FindString(AlumniObj.Gender)
        dtpBirthday.Value = AlumniObj.Birthday
        cboStatus.Text = AlumniObj.CivilStatus
        cboProgram.SelectedIndex = cboProgram.FindString(AlumniObj.Program)
        cboBatchNo.Text = AlumniObj.Batch
        cboSection.Text = AlumniObj.Section
        pbPhoto.BackgroundImage = AlumniObj.Base64ToImage(AlumniObj.Photo)
        'Load Alumni Address
        txtLotNo.Text = AddressObj.LotNo
        txtStreet.Text = AddressObj.Street
        cboBarangay.Text = AddressObj.Barangay
        txtCity.Text = AddressObj.City
        txtProvince.Text = AddressObj.Province
        txtZipCode.Text = AddressObj.ZipCode
        'Load Contact Points
        txtEmail.Text = ContactObj.Email
        txtContact.Text = ContactObj.Contact
        'Load Employment

        If EmploymentObj.IsEmployed = 1 Then
            rbYes.Checked = True
        Else
            rbNo.Checked = True
        End If
        txtEmployer.Text = EmploymentObj.Employer
        txtEmployerAddress.Text = EmploymentObj.Address
        cboLength.Text = EmploymentObj.Length
        cboType.Text = EmploymentObj.Type
        txtPosition.Text = EmploymentObj.Position
    End Sub
